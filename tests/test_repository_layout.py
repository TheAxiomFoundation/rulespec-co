...
ALLOWED_ROOT_DIRS = {".axiom", ".github", "bulk", "data", "programs", "tests", "co"}
ALLOWED_ROOT_FILES = {
    ".gitignore",
    "CLAUDE.md",
    "README.md",
...
def test_only_colombia_namespace_present() -> None:
    """Tanzania mainland scope: the only jurisdiction directory is co/."""
    names = {d.name for d in jurisdiction_dirs()}
    assert names <= {"co"}, f"unexpected jurisdiction dirs: {names - {'tz'}}"


def test_tz_content_buckets_exist() -> None:
    for marker in ("statutes", "regulations", "policies"):
        assert (ROOT / "co" / marker).is_dir(), f"missing co/{marker}"
...
def test_oracle_index_is_tz_scoped() -> None:
    ...
    assert payload["jurisdiction"] == "co"
